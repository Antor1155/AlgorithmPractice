...
class UndirectedGraph:
    ...
    def dfs_visit(self, i, dfs_timer, discovery_times, finish_times, 
        ...
        # your code here
        set_of_adj = self.get_neighboring_vertices(i)
        for k in set_of_adj:
            if (discovery_times[k] != None and not finish_times[k]):
                dfs_back_edges.append([i, k])
                continue
            elif(discovery_times[k] !=None and finish_times[k]):
                continue
            dfs_tree_parent[k] = i
            self.dfs_visit(k, dfs_timer, discovery_times, finish_times, 
                        dfs_tree_parent, dfs_back_edges)
            
        finish_times[i] = dfs_timer.get() 
        self.finished_sorted_list.insert(0, i) 
        dfs_timer.increment()
    ...
    def dfs_traverse_graph(self):
        ...
        return (dfs_tree_parents, non_trivial_back_edges, discovery_times, finish_times, self.finished_sorted_list)
...
def num_connected_components(g): # g is an UndirectedGraph class
    ...
    parents, non_trivial_back_edges, discovery_times, finish_times, finished_sorted_list = g.dfs_traverse_graph()
    
    count = 0
    while len(finished_sorted_list) > 0:
        
        ele = finished_sorted_list[0]
        del finished_sorted_list[0]
        
        to_remove = []
        for i in finished_sorted_list:
            if discovery_times[i] > discovery_times[ele] and finish_times[i] < finish_times[ele]:
                to_remove.append(i)
            
        for x in to_remove:
            finished_sorted_list.remove(x)
...
def find_all_nodes_in_cycle(g): # g is an UndirectedGraph class
    ...
    cycle_compo = []
    for i in non_trivial_back_edges:
        if(i[0] not in cycle_compo):
            cycle_compo.append(i[0])
        if(i[1] not in cycle_compo):
            cycle_compo.append(i[1])
            
    for k in cycle_compo:
        if parents[k] and parents[parents[k]] in cycle_compo:
            cycle_compo.append(parents[k])
    return set(cycle_compo)
