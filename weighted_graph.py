class DisjointForests:
    ...
    def union(self, j1, j2):
        ...
        if j1_root != j2_root:
            if self.rank[j1_root] < self.rank[j2_root]:
                self.parents[j1] = j2_root
            else:
                self.parents[j2_root] = j1_root
                if self.rank[j1_root] == self.rank[j2_root]:
                    self.rank[j1_root] = self.rank[j1_root] + 1
...
def compute_scc(g, W):
    ...
    # your code here
    g.sort_edges()
    edges_with_weight = g.edges
    for i in edges_with_weight:        
        
        if d.parents[i[0]] == None: 
            d.make_set(i[0])
        if d.parents[i[1]] == None:
            d.make_set(i[1])
...
def compute_mst(g):
    ...
    for i in g.edges:
        ...
        if d.find(i[0]) != d.find(i[1]):
            d.union(i[0], i[1])
            mst_edges.append(tuple(i))
    
    mst_weight = 0
    for i in mst_edges:
        mst_weight = mst_weight + i[2]
    print("mst weight is" ,mst_weight)
